    ...
    <div className="w-full bg-gray-100 min-h-screen">
      ...
      <main className="container mx-auto px-4 py-8 flex flex-row justify-between items-start">
        <div className="bg-white shadow-md rounded-lg p-6 mb-6 w-full max-w-3xl">
          <h1 className="text-2xl font-bold text-gray-800 mb-4">Restaurant Details</h1>

          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-800">Gas Stove Usage</h2>
            {restaurant?.gasOrElectricStove ? (
              <div className="bg-green-300 rounded-md px-3 py-1 mt-2">Uses gas stove</div>
            ) : (
              <div className="bg-red-300 rounded-md px-3 py-1 mt-2">Does not use gas stove</div>
            )}
          </div>

          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-800">Green Energy Usage</h2>
            <div className={`rounded-md px-3 py-1 mt-2 ${restaurant?.greenEnergy ? 'bg-green-300' : 'bg-red-300'}`}>
              {restaurant?.greenEnergy}% green
            </div>
          </div>

          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-800">Food Waste Dealing</h2>
            {restaurant?.foodWasteDealing ? (
              <div className="bg-green-300 rounded-md px-3 py-1 mt-2">Deals with food waste</div>
            ) : (
              <div className="bg-red-300 rounded-md px-3 py-1 mt-2">Does not manage food waste</div>
            )}
          </div>

          <div className="mb-4">
            <h2 className="text-xl font-bold text-gray-800">Ingredients Used</h2>
